...
void rl_DrawText(const char *text, int posX, int posY, int fontSize, Color color)
{
    ...
    if (GetFontDefault().texture.id != 0)
    {
        ...
        el_DrawTextEx(GetFontDefault(), text, position, (float)fontSize, (float)spacing, color);
    }
}

// Draw text using Font
// NOTE: chars spacing is NOT proportional to fontSize
void el_DrawTextEx(Font font, const char *text, Vector2 position, float fontSize, float spacing, Color tint)
{
...
void DrawTextPro(Font font, const char *text, Vector2 position, Vector2 origin, float rotation, float fontSize, float spacing, Color tint)
{
    rlPushMatrix();
        ...
        el_DrawTextEx(font, text, (Vector2){ 0.0f, 0.0f }, fontSize, spacing, tint);
...
static Font LoadBMFont(const char *fileName)
{
    ...
    Image imFont = rl_LoadImage(imPath);
